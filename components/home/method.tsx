            ...
            <Reveal key={title} delay={index * .06}>
              <article className={`relative mb-8 grid md:grid-cols-2 ${index % 2 ? "" : "md:text-right"}`}>
                ...
                <div className={`premium-card ml-12 min-h-[230px] p-8 md:ml-0 md:w-[calc(100%-56px)] ${index % 2 ? "md:col-start-2 md:ml-14" : "md:mr-14"}`}>
                  <div className={`flex items-center gap-4 ${index % 2 ? "" : "md:justify-end"}`}><span className={`grid size-11 place-items-center rounded-2xl border ${index % 2 ? "border-electric/30 bg-electric/[.07] text-electric" : "border-accent/20 bg-accent/[.05] text-accent"}`}><Icon size={19} strokeWidth={1.4} /></span><span className={index % 2 ? "text-[9px] tracking-[.24em] text-electric" : "text-[9px] tracking-[.24em] text-accent"}>{number}</span></div>
