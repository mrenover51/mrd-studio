    ...
    <header className={`fixed inset-x-0 top-0 z-50 px-3 transition-[padding,transform] duration-700 ease-out ${scrolled ? "translate-y-3 sm:px-5" : "translate-y-5 sm:px-7"}`}>
      <div
        className={`relative mx-auto flex max-w-[1320px] items-center justify-between overflow-hidden rounded-full border px-5 transition-[height,max-width,background-color,border-color,box-shadow] duration-700 ease-out sm:px-7 ${
          scrolled
            ? "h-[64px] max-w-[1160px] border-accent/20 bg-[#10131a]/72 shadow-[0_18px_65px_rgba(0,0,0,.55),inset_0_1px_0_rgba(255,255,255,.075),inset_0_-1px_0_rgba(0,217,255,.05)] backdrop-blur-[28px]"
            : "h-[76px] border-accent/15 bg-[#10131a]/48 shadow-[0_24px_80px_rgba(0,0,0,.32),inset_0_1px_0_rgba(255,255,255,.06)] backdrop-blur-[20px]"
        }`}
      >
        ...
        />
        <span className="pointer-events-none absolute inset-x-[9%] bottom-0 h-px bg-gradient-to-r from-transparent via-accent/20 to-transparent" />
        ...
        <Link href="/#accueil" aria-label="MRD Studio — Accueil" className="group relative z-10 block h-[52px] w-[78px] shrink-0 overflow-hidden">
          <Image src="/logo.png" alt="MRD Studio" fill priority quality={100} sizes="78px" className="object-contain drop-shadow-[0_0_7px_rgba(0,217,255,.12)] transition-[filter,opacity] duration-700 group-hover:brightness-110 group-hover:drop-shadow-[0_0_11px_rgba(0,217,255,.24)]" />
        </Link>
        ...
        <Link href="/contact" className="group relative z-10 hidden h-10 items-center gap-2 overflow-hidden rounded-full border border-accent/40 bg-gradient-to-b from-[#42f5ff] to-accent px-5 text-[9px] font-semibold uppercase tracking-[.12em] text-ink shadow-[inset_0_1px_0_rgba(255,255,255,.6),inset_0_-8px_18px_rgba(30,31,80,.18),0_8px_28px_rgba(0,217,255,.2)] transition-all duration-500 hover:-translate-y-0.5 hover:border-electric/70 hover:from-[#9a83ff] hover:to-electric hover:shadow-[inset_0_1px_0_rgba(255,255,255,.75),0_12px_38px_rgba(122,92,255,.3)] sm:flex">
          <span className="pointer-events-none absolute left-[18%] top-0 h-1/2 w-2/3 rounded-full bg-white/35 blur-md" />
          <motion.span animate={{ x: ["-180%", "360%"] }} transition={{ duration: 4.5, repeat: Infinity, repeatDelay: 5 }} className="pointer-events-none absolute inset-y-[-30%] w-8 rotate-12 bg-white/25 blur-sm" />
